...
## Workflow per file (per `docs/INSTRUCTION.md` §18, §30)

For every file, in order:

1. **Understand** — read the file fully, list inputs/outputs, global
   `#@param` dependencies, side effects, and execution order that must
   be preserved.
2. **Preserve** — write down (in the PR description) the behavior that
   must not change: output file naming, default values, log wording
   meaning, CLI-equivalent flags.
3. **Design** — decide function grouping, dataclass grouping (if any),
   naming fixes, and English-translation scope (§31).
4. **Refactor** — small, focused commit(s). Do not mix an English
   translation pass with a structural refactor unless both are small
   enough to review together (§19, §31.2).
5. **Verify** — syntax check, import check, call-site check, compare
   behavior against the pre-refactor version.
6. **Summarize** — end of session: files/functions changed, any
   intentional behavior change + why, anything left out of scope.
7. **Track** — tick the corresponding checkbox in `PROGRESS.md` and
...
- **Mixed Indonesian/English** in logs, comments, and print statements
  across most files (§31 requires English-only for anything touched).
- **`#@param` globals used directly deep in logic** instead of being
  converted once into a config object at a clear boundary (§3, §4.2).
  A handful of files (`download_video_via_ytdlp_or_aria2c.py`,
...
- **Generic/inline `ffmpeg`/`ffprobe`/`yt-dlp` dependency-install
  ...
  should still follow the idempotency rule in §4.4.
- **Bare `except Exception` / silent `pass`** in a few files — needs
  case-by-case review per §8.
- **Inconsistent success/failure return values** — some functions
  ...
  (§10.1), since these files don't import each other.
...
| 2 | `extract_audio_from_video.py` | Mixed language; codec→extension mapping and ffprobe codec detection could be split into a named helper (§1.3). |
| 3 | `fix_mono_audio_channel.py` | Uses `!{cmd}` shell magic instead of `subprocess.run([...])` (§23, "avoid unsafe shell construction"); heavy Indonesian logging; audio-vs-video branch could be split into two named helpers. |
...
| # | File | Notable issues to address |
|---|------|----------------------------|
| 1 | `download_file_via_requests_or_aria2c.py` | Two backends (`requests` vs `aria2c`) already reasonably separated — check against §5.5 (backend selection consistency) rather than restructure from scratch; some duplicated formatting helpers (`format_bytes`, progress bars) between the two engines. |
...
| 3 | `download_video_refactored_podjav.py` | Already has type hints, docstrings, English logging — treat as a **verification pass** (confirm it matches §2–§12 fully) rather than a full rewrite. Uses Indonesian in some log message *contents* (not the code) — check if that's deliberate localized output per §31.3 or should be translated. |
...
| # | File | Notable issues to address |
|---|------|----------------------------|
| 1 | `delete_files_by_filter.py` | `should_delete()` relies on several `#@param` globals directly instead of parameters (§4.1); `debug_mode` mixed into normal logging flow instead of using DEBUG level (§9.4). |
...
| 3 | `move_or_copy_files_with_filter.py` | Long parameter list on `move_or_copy_path()` (10+ args) — good candidate for a config dataclass per §3.1; nested closures (`should_process`, `safe_operation`) make local reasoning harder, consider extracting. |
| 4 | `rename_files_sequentially.py` | Small and simple; mixed language; padding-digit logic (`if total_files >= 100... elif >= 10... else...`) has a redundant branch (`>=10` and `else` both return `2`) worth flagging in the PR rather than "fixing" silently (§1.1 — confirm intent first). |
...
| # | File | Notable issues to address |
|---|------|----------------------------|
| 1 | `fetch_assemblyai_transcript_result.py` | Reasonably clean already; several `save_*` functions duplicate the same open/write/log pattern — could extract a small shared writer, but weigh against §5.4 (avoid over-abstracting a 3-line pattern). |
...
| 3 | `transcribe_audio_via_assemblyai.py` | Longest/most complex file in this folder; heavy Indonesian logging; `main()` does upload + transcribe + poll + save + cleanup all inline — strong candidate for splitting into named stage functions per §5.1/§5.2, but do this as more than one incremental PR (§19). |

### 6. `uploaders/`

| # | File | Notable issues to address |
|---|------|----------------------------|
| 1 | `upload_video_to_facebook.py` | Largest file in the repo; imports scattered inside functions instead of at top of file (§16); scheduling logic (`get_batch_schedule_times`, `get_schedule_timestamp_iso_or_unix`) duplicates timezone/validation logic — consolidate carefully, this is the highest-risk file in the repo for accidental behavior change (real API side effects). Treat as **last** file overall, after the workflow has been validated on lower-risk files. |
| 2 | `upload_video_to_youtube.py` | `TEMPLATE_FOOTER` built from `#@param` values at module load time before `main()` runs — check against §4.4 (persistent runtime state / rebuild-per-run) since Colab re-runs can leave stale globals; mixed Indonesian/English. |
...
| 2 | `mux_audio_into_video.py` | Small and clean; `mode` param ("replace"/"add") is a good existing example of the §5.5 "validated string, not enum" pattern — keep as-is. |
...
| 4 | `mux_subtitle_into_video_enhanced.py` | Best-structured file in this folder — already does validation, verification-after-mux, and structured result dicts. Mostly needs the English-translation pass (§31) and a check that `mux_subtitle()`'s responsibilities (validate + build cmd + run + verify + cleanup) aren't too many for one function (§1.2). |
