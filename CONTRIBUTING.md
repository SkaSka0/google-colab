...
## Before touching a specific sub-project

Some folders (e.g. `Telegram-Leecher/`, `Telegram-Fetcher/`) have their
own:
...
Check these before starting work, so you don't duplicate something
already in progress or reintroduce something intentionally deferred.

## Making changes

- Keep PRs small and focused — one responsibility per PR, per
  `docs/INSTRUCTION.md` §19 and §30.
- Do not change existing behavior, defaults, or output formats unless
  ...
  PR description.
- Follow the naming conventions in `docs/INSTRUCTION.md` §2 and §12
...
- [ ] I checked the relevant sub-project's `ROADMAP.md` / `PROGRESS.md`, if any
